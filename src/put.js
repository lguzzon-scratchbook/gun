...
Gun.chain.put = function(data, cb, as){ // I rewrote it :)
	...
	if('function' == typeof data){ data((d)=> { as.data = d; this.put(u,u,as) }); return this }
	if(!as.soul){ return get(as), this }
	...
	(function walk(){
		...
		if(!(v = valid(d)) && !(g = Gun.is(d))){
			if(!Object.plain(d)){ ran.err(as, "Invalid data: "+ check(d) +" at " + (as.via.back((at)=> {at.get && tmp.push(at.get)}, tmp = []) || tmp.join('.'))+'.'+(to.path||[]).join('.')); return }
		...
		if(k && v){ at.node = state_ify(at.node, k, s, d) } // handle soul later.
			...
			function resolve(msg, eve){
				...
				if(!soul && !at.link['#']){ // check soul link above us
					(at.wait || (at.wait = [])).push(()=> { resolve(msg, eve) }) // wait
					return;
				}
				if(!soul){
					soul = [];
				...
				delete as.wait[id];
				cat.wait && setTimeout.each(cat.wait, (cb)=> { cb && cb() });
				as.ran(as);
			};
			// ---------------
		}
...
function ran(as){
	...
	(tmp = ()=> { // this is not official yet, but quick solution to hack in for now.
		if(!stun){ return }
		ran.end(stun, root);
		setTimeout.each(Object.keys(stun = stun.add||''), (cb)=> { if(cb = stun[cb]){cb()} }); // resume the stunned reads // Any perf reasons to CPU schedule this .keys( ?
...
var u, empty = {}, noop = ()=> {}, turn = setTimeout.turn, valid = Gun.valid, state_ify = Gun.state.ify;
var iife = (fn,as)=> {fn.call(as||empty)}
